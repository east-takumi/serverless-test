...
def _run_smoke_test(client, state_machine_arn: str, region: str) -> Dict[str, Any]:
    ...
    history_events = client.get_execution_history(executionArn=execution_arn).get("events", [])
    ...
    stack_name = os.environ.get("SAM_STACK_NAME", "stepfunctions-local-testing")
    environment = os.environ.get("ENVIRONMENT", "local")
    account_id = os.environ.get("LOCAL_AWS_ACCOUNT_ID", "123456789012")

    expected_functions = {
        f"arn:aws:lambda:{region}:{account_id}:function:{stack_name}-ProcessState1-{environment}",
        f"arn:aws:lambda:{region}:{account_id}:function:{stack_name}-ProcessState2-{environment}",
        f"arn:aws:lambda:{region}:{account_id}:function:{stack_name}-ProcessState3-{environment}",
    }

    missing_functions = sorted(expected_functions.difference(set(scheduled_functions)))
    if missing_functions:
        raise RuntimeError(
            "State machine execution did not schedule all expected Lambda functions: "
    ...
    return {
        ...
        "scheduledFunctions": scheduled_functions,
    }
...
def main() -> int:
    ...
    try:
        ...
        smoke_test_result = _run_smoke_test(client, state_machine_arn, region)
        ...
        print("  Lambda functions scheduled:")
        for resource in smoke_test_result["scheduledFunctions"]:
            print(f"    - {resource}")
